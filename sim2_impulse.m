...
% Langkah
% Buat kontroler P, naikkan sampai osilasi stabil tapi ada sedikit overshoot
% Kp = 0.25
% Estimasi awal Ki mengikuti time constant, lalu perkecil sampai settling time optimal
% Ti = 0.047
% Estimasi awal Kd dengan 1/10 Ki, naikkan hingga overshoot hilang, lanjutkan dengan adjust ulang Ti
% Ti = 0.040, Td = 0.057
...
clf();
axis([0 ft min([h1 h2 h3 h4]) max([h1 h2 h3 h4])]);
stairs(t2,p2,'b'); hold on; %blue
stairs(t3,p3,'k'); %black
%stairs(t4,p4,'g'); %green
hold off;
title(['Posisi pada SP = ' num2str(SP) ', Kp = ' num2str(Kp) ...
	', Ti = ' num2str(Ti) ', Td = ' num2str(Td) ', dt = ' num2str(dt)])
